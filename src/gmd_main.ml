...
let search_multi param =
    ...
    (fun corpus_id ->
      ...
      Corpus.bounded_search
        ...
        (fun graph_index sent_id _ pos_in_graph nb_in_graph matching x -> 
          ...
          request clust_item_list corpus in
      let full_clusters = Clustered.map (fun l -> { Session.data = Array.of_list (List.rev l); next = 0}) clusters_list in
      (full_clusters, status, ratio)
    ) corpus_id_list in

  let _top_clusters = Clustered.build_layer
    (fun (full_clusters, _, _) -> full_clusters)
    (fun (full_clusters, status, ratio) ->
      match status with
      | "complete" -> Some (sprintf "%d" (Clustered.cardinal Session.cluster_size full_clusters))
      | "timeout" -> Some (sprintf "%d in %.2f (TO)" (Clustered.cardinal Session.cluster_size full_clusters) (100. *. ratio))
      | "max_results" -> Some (sprintf "1000 in %.2f" (100. *. ratio))
      | _ -> assert false
    )
    Session.empty_cluster 
    result_list in
  let (json_clusters, clusters) = match Clustered.sizes Session.cluster_size _top_clusters with
    | [xxx] ->
      let arr = Array.of_list (String_opt_map.fold (fun k v acc -> (k,v) :: acc) xxx []) in
      cluster_sort arr;
      (("cluster_array", json_values_sizes arr), _top_clusters)
    | _ -> failwith "TODO: Dim not handled" in

  let session = {
    Session.corpus_id = "";
    Session.request = failwith "TODO";
    last=Unix.gettimeofday ();
    clusters;
  } in
  Client_map.t := String_map.add uuid session !Client_map.t;
  ...
  let json_output = `Assoc [
      ...
      ("status", `String "NO");
      ("ratio", `Float 0.42);
      ("nb_solutions", `Int (Clustered.cardinal Session.cluster_size _top_clusters));
      ("pivots", `List []);
      ("time", `Float (Unix.gettimeofday () -. start_time));
      json_clusters
    ] in
